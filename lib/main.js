...
module.exports = {
  ...
  activate(_state, { signal, cause } = {}) {
    ...
    if (lumine.config.get("about.showOnStartup") && cause?.type !== "workspace-opener") {
      // Rendering About imports Etch and builds the complete view. Keep the
      // opener and commands synchronous, then open the startup pane after the
      // activation measurement has ended. The generation guard cancels a
      // queued open when the package is deactivated or reloaded first.
      queueMicrotask(() => {
        if (!this.active || generation !== this.activationGeneration || signal?.aborted) return;

        // Open the model directly. Going through the package opener from
        // activation would re-enter package activation in older hosts.
        const aboutView = this.ensureModel().deserialize();
        void waitForSignal(lumine.workspace.open(aboutView), signal).catch((error) => {
          if (this.active && generation === this.activationGeneration && !signal?.aborted) {
            console.error(`Failed to open About: ${error.message}`);
          }
        });
      });
    }
